...
| `/affordable-housing/` | Affordable Housing | WordPress page; program detail | Header Programs; footer Our Programs; homepage program cards | Program detail | Shared program-family template | Program hero, split intro with program logo, application card, approved featured video, image CTA, Other Programs cards | Privacy-friendly click-to-load YouTube video and external needs-assistance application; no local form | Body is primarily HTML; global image/nav/button/heading/icon/social widgets | PlanStreet public form, Kindful, YouTube; card/button hover | Medium | **COMPLETE** |
...
| `/hope-in-action/` | Social Media (display hero: Hope In Action) | WordPress page; live social feed | Header Hope In Action; footer Quick Links | Social feed | Unique feed page; reuses global hero/CTA language only | Image hero, social profile buttons, live feed, image CTA | Native Instagram Login API feed and client-side Load More | Primarily HTML plus global widgets | Server-only Instagram API, Instagram post links, social profiles, Kindful | High | **COMPLETE** |
...
| `/volunteer/` | Volunteer | Post-migration Next.js enhancement | Header and footer Get Involved links; volunteer CTAs on Contact, Board & Staff, and Hope In Action | Volunteer conversion | Route-specific page using the accepted global shell | Volunteer hero, mission intro, opportunity cards, approved volunteer video, three-step expectation flow, Typeform CTA | Privacy-friendly click-to-load YouTube video and external volunteer interest form; no local form | Not applicable; added after the Elementor migration | Typeform volunteer intake; YouTube; generated illustrative route-specific imagery | Medium | **COMPLETE** |
...
| `/feed/` and feed variants | Historical WordPress RSS output; no published posts currently exist | **EXCLUDE** as a page; the verified legacy `/feed/` URL permanently redirects to `/news-media/`. Reassess feed functionality only if headless posts are later enabled. |
...
| `#content`, `#trailer-form`, query strings, slash/no-slash duplicates | Fragment or duplicate variants of canonical pages | **EXCLUDE** as separate pages; retain meaningful anchors. The Search Console-confirmed homepage parameters `trk`, `et_fb`, `PageSpeed`, and `et_core_page_resource` are removed by a homepage-only permanent redirect while unrelated query parameters remain available. |
...
External flows are dependencies, not local migration pages: Kindful/Bloom donations, Typeform volunteer/partnership/application flows, PlanStreet public assistance intake, the Instagram API, YouTube, Candid, publisher articles, social profiles, and partner websites.
...
| Social feed | `/hope-in-action/` | Native Instagram-backed feed and unique social layout | Page-specific implementation around shared hero/CTA primitives. |
...
| Native Instagram social feed | Hope In Action only | **D** | Keep Meta access server-only behind a page-specific provider boundary and fallback. |
...
- Hope In Action now renders normalized Instagram media in React and owns its bounded Load More behavior.
...
| Instagram API with Instagram Login | Live Hope In Action social feed | Server-only token dependency with cached normalized results, operational token refresh, and a usable failure fallback. |
...
4. At inventory time, Juicer was the only live social-feed dependency and could change markup, height, network timing, and content independently of WordPress; the accepted Next.js baseline now uses the native Instagram provider documented below.
...
8. At inventory time, external Typeform, PlanStreet, Kindful, Candid, Juicer, social, and publisher links required availability/privacy/ownership review; Juicer was subsequently replaced by the approved native Instagram integration.
...
1. **Affordable Housing — COMPLETE** — established the program family (`ProgramHero`, split intro/application card, image CTA, Other Programs); the later approved HGG walkthrough uses the shared click-to-load featured-video presentation.
...
### Post-migration press-library refresh — 2026-08-12

The organization audit identified missing press coverage after the source-parity migration. A current publisher search added three verified, non-duplicative entries: County Materials' May 6, 2026 partnership feature, the American Land Title Association's December 17, 2024 grant-and-family-impact story, and South Lake Tablet's April 16, 2024 poverty simulation coverage. The page now contains 15 chronologically ordered publisher links, with the 2026 item as the featured lead. The truncated hero sentence was corrected, and matching description/Open Graph metadata was added. Four-width rendered QA at 1440/768/390/320px and all 15 secure external-link contracts pass. Older archive results that duplicated existing project coverage, announced former personnel, or only advertised past events were reviewed but not added.

### News coverage update — 2026-08-12

Three additional publisher-verified stories were added: South Lake Tablet's July 19, 2026 Lake County Supervisor of Elections fundraiser coverage, Duke Energy's April 28, 2026 Foundation grant announcement, and South Lake Tablet's April 19, 2026 Chamber breakfast recap. The News & Media route now contains 18 entries. Every entry carries an ISO publication date and the route sorts that field descending at render time, ensuring the newest story remains first while retaining all existing coverage. Desktop and mobile rendered checks at 1440px and 390px preserve the accepted featured-card, grid, CTA, and footer geometry with no clipping or horizontal overflow. All 16 interaction assertions pass, including all 18 secure publisher links and the automated newest-first date-order check.
...
## Historical Milestone 14 — Hope In Action and former Juicer feed QA

This section records the production-source contract and the original Milestone 14 implementation for audit history. It does **not** describe the active Next.js provider; the current native Instagram architecture begins under “Native Instagram provider enhancement” below.

`/hope-in-action/` is implemented as a route-scoped social-feed page using the accepted `SiteHeader`, `SiteFooter`, global focus treatment, and floating donation control. It preserves the production image hero, source copy, four social-profile controls, 1480px feed section, elevated feed wrapper, final Kindful/Typeform CTA, shared campaign image crops, and responsive page spacing. Production's `new-home-banner.jpg` is byte-identical to accepted `public/images/give-banner.jpg`, so the local route reuses that file. At the historical milestone, no changing provider post image or video was copied into the repository, and no asset was generated or substituted.

At Milestone 14, production used Juicer's standard public client embed, not an iframe, WordPress shortcode renderer, private API client, or locally owned card presentation. The audited page contract was `<ul class="juicer-feed" data-feed-id="findfeedrestore">` plus `https://www.juicer.io/embed/findfeedrestore/embed-code.js`; the loader identified the public provider feed internally as ID `454635`. It injected the Juicer 2.0 stylesheet/runtime, fetched `/api/feeds/findfeedrestore.html`, supplied all card/media/caption markup, dispatched lifecycle DOM events, posted its own page-view signal, and owned Load More, modal/external-post, media, and responsive-grid behavior. No provider credential was present or required.

The former `src/components/hope-social-feed.tsx` adapter loaded the route-specific embed through Next.js `Script` with `lazyOnload`, gave the provider one feed node, listened for its lifecycle event, and used its initialize/remove hooks for App Router cleanup. That file and all associated active runtime code were removed by accepted commit `441e86c` after the native provider passed QA.

The historical provider feed established container-query transitions at 640, 900, and 1100px. The native implementation deliberately retains those accepted 758, 1018, and 1218px viewport transitions, one/two/three/four-column progression, 12px gap, contained media, captions, profile metadata, Load More, and secure external Instagram links.

At the historical milestone, live content was treated separately from static layout during visual QA. Changing post order, media, captions, media availability, or resulting card height was not a static fidelity defect. Hero geometry, copy, section boundaries, wrapper width/padding/background, CTA, and footer transition remained static targets.
...
At that milestone, dedicated automation passed the historical provider's loader/runtime, Load More, secure-link, media-containment, breakpoint, remount, overflow, and blocked-script checks. The same script now certifies the active native Instagram contract described below.

The original milestone recorded exact Home and News regressions without shared application changes. Its proposed future native-provider option was subsequently completed and accepted at `441e86c`.

### Native Instagram provider enhancement

The approved provider enhancement replaces the Juicer runtime described above while preserving the certified Hope In Action shell. `src/lib/instagram-provider.ts` is the server-only environment boundary, `src/lib/instagram-feed.ts` fetches and normalizes provider data, `src/components/hope-instagram-feed.tsx` owns loading/failure selection, and `src/components/instagram-feed.tsx` owns the native card grid and bounded Load More interaction. Runtime code reads only the server variables `INSTAGRAM_ACCESS_TOKEN` and `INSTAGRAM_ACCOUNT_ID`; neither name is prefixed with `NEXT_PUBLIC_`, and the optimized client-bundle scan contains no token value. Juicer's adapter, route script loader, lifecycle hooks, feed markup, and route CSS were removed only after the native implementation passed provider, browser, visual, and fallback QA.

The current Instagram API with Instagram Login contract uses `https://graph.instagram.com/v26.0/me` for the authorized professional profile and `https://graph.instagram.com/v26.0/me/media` for recent media. Requests use a bearer authorization header and ask only for normalized profile/media fields. Media collections return a `data` array plus cursor-based `paging` (`before`/`after` and provider `next`/`previous` URLs when applicable); this bounded implementation intentionally consumes only the first 24-item page. It filters malformed entries and supports `IMAGE`, `VIDEO` (including Reel media returned as video), and `CAROUSEL_ALBUM`; the first carousel implementation uses the parent cover, and video uses the returned thumbnail without autoplay. Captions render only as text. Every card links to its provider permalink with `_blank` and `noopener noreferrer`. Contract references are the official Meta [Instagram Login getting-started guide](https://developers.facebook.com/documentation/instagram-platform/instagram-api-with-instagram-login/get-started), [Instagram User media reference](https://developers.facebook.com/documentation/instagram-platform/reference/instagram-user/media), [Instagram Media reference](https://developers.facebook.com/documentation/instagram-platform/reference/instagram-media), and [official Instagram API Postman workspace](https://www.postman.com/meta/workspace/instagram/overview).

Next.js caches the profile and bounded media responses for 900 seconds. Sixteen posts render initially and each Load More activation reveals up to eight more from the already cached 24-post result; provider cursors and tokens never reach the browser. The actual authorized account check returned 24 valid recent image posts with captions, media URLs, and Instagram permalinks. The current account had no recent video or carousel item, so those two normalization/presentation paths are covered through provider-seam tests rather than represented as a live-account claim.

Missing configuration, timeout/network failure, token failure, non-JSON/provider error, account mismatch, an empty result, and malformed media all return a safe result instead of breaking the route. The visitor sees an understated status plus the known public Instagram profile link; raw Meta errors and credentials are never exposed. Browser QA separately verified both missing-environment and invalid-token fallbacks at 390px, including a complete page shell and zero horizontal overflow. Server diagnostics contain only a reason plus optional HTTP status/provider code.

Meta documents long-lived Instagram User access tokens as valid for approximately 60 days. An eligible, unexpired token that is at least 24 hours old and retains `instagram_business_basic` can be refreshed through `https://graph.instagram.com/refresh_access_token` with `grant_type=ig_refresh_token`; an expired token cannot be refreshed. Because Vercel environment variables are immutable application configuration and a refreshed token must be persisted securely, this implementation does not attempt to rotate that variable at runtime. Production operations must refresh before expiry, store the replacement through the controlled Vercel environment workflow, redeploy, run the safe provider verifier, and alert on the provider's token diagnostic so expiration cannot fail invisibly.

Native visual QA passes at 1440, 1024, 768, 430, 390, 375, 360, and 320px. The certified 1480px section, elevated wrapper, 12px gaps, exact 1/2/3/4-column transitions at 758/1018/1218px, square contained media, CTA/footer relationship, and mobile one-column containment remain intact. Standard production/local heights are 4917/4897, 6147/6172, 7455/7450, and 12994/13086px; dedicated mobile heights are 13177/13669, 12994/13086, 12930/13099, 12890/13159, and 13013/13210px. The remaining cumulative variance is changing live captions/media and native card text flow, not static shell or breakpoint drift.

Eight provider unit tests and eight dedicated browser checks pass: current host/version/fields, bounded cached requests, safe bearer handling, account verification, all supported normalized/presentation types, malformed/error behavior, server-only configuration, 16-to-24 Load More, secure links/focus, square media, breakpoint boundaries, five-width mobile containment, and App Router remount. Homepage and News & Media regression captures/interactions pass without shared application changes. ESLint, strict TypeScript, `git diff --check`, and the optimized production build pass after Juicer removal.

The focused cutover-baseline refresh at accepted commit `441e86c` repeated the live provider check, all provider/browser/fallback checks, shared Hope interactions, eight visual widths, Home regression, and News & Media regression. Native local heights exactly match the accepted evidence at every width. The legacy WordPress source's client-side feed did not populate during the refresh captures, so the new artifacts document matching static shell geometry plus an empty historical-provider wrapper rather than treating absent third-party source content as a native-feed regression. A narrow QA-only image-normalization change uses canvas compositing when a dynamic source creates more than 10,000px of height variance; application rendering is unchanged.
...
All 16 canonical routes pass the repository-local crawl, standard four-width visual certification, dedicated five-width mobile certification at 430/390/375/360/320px, route interaction suite, canonical/title/noindex checks, internal-link audit, accessibility smoke audit, local-asset audit, and 404 verification. Dedicated Resend/form, native Instagram, media, and legal checks pass. The complete evidence, per-route matrix, mobile matrix, redirect matrix, dependency inventory, asset/build notes, certification fixes, and categorized launch items are maintained in `docs/FULL_SITE_CERTIFICATION.md`.
...
The post-launch Search Console cleanup added semantic permanent redirects from historical `/about/` to `/board-staff/` and `/feed/` to `/news-media/`. A narrowly matched `src/proxy.ts` removes only the obsolete homepage parameters `trk`, `et_fb`, `PageSpeed`, and `et_core_page_resource`; normal homepage requests and unrelated query parameters pass through unchanged. Canonical pages remain the only sitemap entries, and retired WordPress runtime assets remain unavailable.
